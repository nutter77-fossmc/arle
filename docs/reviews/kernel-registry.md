...
| linear | `MarlinW4Hybrid` | Marlin W4+W4A8 prefill fuse (`gemm/marlin_w4a8_kernel.cu`) | SM_80+ · W4A16 · **prefill batch>1 only**, opt-in `INFER_HYBRID_W4A8_PREFILL_ENABLED` | hardcoded; alignment fallback demotes via `log::trace!` (`linear.rs:139`, invisible at default log) | tensor-core prefill | — at SOTA for prefill; default OFF, env-gated | — |
