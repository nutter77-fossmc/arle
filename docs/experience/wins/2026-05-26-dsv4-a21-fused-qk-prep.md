...
| Run | Artifact | Result |
| --- | --- | --- |
| Build | `dsv4-a21-fuse-qk-build-20260526/build.log` | `cargo build --release -p infer --features cuda,nccl --bin infer` passed in 6m52s |
| Smoke split | `dsv4-a21-qk-split-smoke-max32-20260526` | `max_tokens=32`, elapsed 7.5552s, output `4062 0.0000 ...` |
| Smoke fused | `dsv4-a21-qk-fused-smoke-max32-20260526` | `max_tokens=32`, elapsed 7.5016s, byte-identical output |
| Smoke default | `dsv4-a21-qk-default-smoke-max32-20260526` | unset DSv4 backend/fuse env, env-check resolved `deepep` + `deepgemm`, elapsed 7.5169s, byte-identical output |
| Nsys split | `dsv4-a21-qk-split-nsys-max32-20260526/nsys` | profile request 3.1086s; `dsv4_prepare_q_kernel`: 7352 calls / 23.8502ms; `dsv4_prepare_k_kernel`: 7342 calls / 19.3829ms |
| Nsys fused | `dsv4-a21-qk-fused-nsys-max32-20260526/nsys` | profile request 3.0231s; `dsv4_prepare_qk_fused_kernel`: 7290 calls / 24.2723ms |
