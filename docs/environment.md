...
| `ARLE_DSV4_ROUTE_GROUPED_EXPERTS` | `1` / unset | unset | Enables the route-wise grouped local expert experiment for padded B=1 decode. It removes the local-count D2H readback but regresses the real 8xH20 single-token trace because the route-wise FP4 GEMV over fixed `ep_world * topk` slots is slower than the default packed local expert loop. Keep default-off. |
