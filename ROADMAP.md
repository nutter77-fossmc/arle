...
| P0'' | **DeepSeek V4 readiness prep (parallel product line, #1 next-model priority)** — current truth is V4-only: `crates/deepseek-spec` parses the local `DeepseekV4ForCausalLM` 1B init config/tensors, old V2/V3/nano public paths are deleted, `cpu_serve` has a slow Rust reference forward for 1-token HTTP smoke, and `arle train pretrain-dsv4` seeds from the same V4 1B init checkpoint. CUDA serving performance remains gated on V4 hybrid attention + MoE + MTP kernels and later multi-rank collectives. | [`docs/projects/2026-05-01-deepseek-v4-readiness.md`](docs/projects/2026-05-01-deepseek-v4-readiness.md), [`docs/plans/2026-05-10-dsv4-qwen36-substrate-audit.md`](docs/plans/2026-05-10-dsv4-qwen36-substrate-audit.md), [`docs/plans/dsv4-small-repro.md`](docs/plans/dsv4-small-repro.md) |
...
1. **DeepSeek V4 (DS4)** — highest-priority next model. V4-only spec,
   registry, train bootstrap, and CPU reference HTTP smoke have landed against
   `infer/models/dsv4-mini-1B-init`; CUDA V4 hybrid attention + MoE + MTP
   kernels are the work this lane is converging on. Tracked under P0'' above.
