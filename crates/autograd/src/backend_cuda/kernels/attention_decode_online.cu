// Online-softmax decode-attention kernel for the OPD rollout phase (tape
// disabled).
//
// Improvements over `causal_sdpa_decode_gqa_cache_f32` in attention.cu:
//   1. **Online softmax** (Pellegrini 2018) — a single fused pass over the
//      visible KV positions with running max + denom + numerator in
//      registers, instead of the original score-then-softmax-then-mix that
//      staged the full `scores[visible]` table in shared memory and ran a
//      single-thread softmax at tid==0.
//   2. **Block size matches head_dim** — one element per thread for the QK
//      dot, then warp-level reductions for the cross-thread sum. Qwen3.5-0.8B
//      uses head_dim=256 so we instantiate the kernel at 256 (full
//      utilization, no idle threads).
//
// Launch contract (matches the original kernel for drop-in replacement):
//   - q:        f32 [batch, query_heads, 1, head_dim]
//   - k_cache:  f32 [batch, kv_heads, max_seq, head_dim]
//   - v_cache:  f32 [batch, kv_heads, max_seq, head_dim]
...
// `causal_sdpa_decode_gqa_cache_f32` kernel (online vs two-pass softmax both
// compute exp(s - max); difference is floating-point associativity only).
//
...
__global__ void causal_sdpa_decode_gqa_cache_online_f32_hd256(
    ...
    causal_sdpa_decode_gqa_cache_online_impl<256>(
