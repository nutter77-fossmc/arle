...
// ============================================================================
// GEMM-shape grouped FP8 block-scaled kernels (M-tile = DSV4_BATCH_TILE).
//
// Background: the grouped GEMV kernels above (line 701 / 816) index grid Y by
// `max_count` (one block per token), so each block reads N×K_blk weight once
// and uses it for ONE token. Weight reuse = 1×. Fine at decode (M=1) but
// catastrophic at prefill: 29795-token batch hit 325s wall-clock (67× off SLO)
// because every prefill token streams the entire expert weight matrix from HBM.
// See docs/experience/errors/2026-05-27-dsv4-tp-allreduce-slo-prefill-kill.md.
//
// These kernels mirror dsv4_fp8_gemv_batch_tiled_kernel (line 392): grid Y
// indexes a tile of DSV4_BATCH_TILE tokens per block, the weight load is
// reused across all tokens in the tile (32× reuse), and per-thread M-tile
// accumulators sit in registers. Pair variant computes gate/up outputs from
// the same input row in one pass for ~2× input bandwidth saving.
//
// Drop-in compatible ABI with the grouped GEMV cudaError_t wrappers.
// ============================================================================
    ...
    int scale_cols)
{
    ...
    // Fast path: tile_M <= 4 — matches _tiled_kernel sums4 pattern, lower
    // register pressure when many experts only get a handful of tokens.
    if (tile_M <= 4) {
        float sums4[4];
#pragma unroll
        for (int b = 0; b < 4; ++b) sums4[b] = 0.0f;
...
#pragma unroll
        ...
        for (int sc = 0; sc < scale_cols; ++sc) {
            ...
            int k_end = k_start + block_w;
            if (k_end > K) k_end = K;
            const float sa = dsv4_decode_e8m0(scale_a_row[sc]);
            const float sb = dsv4_decode_e8m0(scale_b_row[sc]);
            for (int k = k_start + tid_in_row; k < k_end; k += threads_per_row) {
                const float wa = dsv4_decode_fp8_e4m3(weight_a_row[k]) * sa;
                const float wb = dsv4_decode_fp8_e4m3(weight_b_row[k]) * sb;
#pragma unroll
                for (int b = 0; b < 4; ++b) {
                    if (b < tile_M) {
...
#pragma unroll
        for (int b = 0; b < 4; ++b) {
            sums_a4[b] = warp_reduce_sum(sums_a4[b]);
            sums_b4[b] = warp_reduce_sum(sums_b4[b]);
            if (lane_id == 0) {
...
#pragma unroll
    for (int b = 0; b < DSV4_BATCH_TILE; ++b) {
        sums_a[b] = warp_reduce_sum(sums_a[b]);
        sums_b[b] = warp_reduce_sum(sums_b[b]);
        if (lane_id == 0) {
...
// GEMM-shape variants: grid Y tiles tokens into DSV4_BATCH_TILE=32 groups for
// weight reuse. Drop-in compatible signatures with the grouped GEMV wrappers.
