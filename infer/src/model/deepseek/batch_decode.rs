...
///
/// Owns the **shared persistent FP8 KV pool** that backs FlashMLA sparse decode
/// (Phase D-4). Allocated once at `create_decode_context` and sized for
/// `num_slots × layers × slot_blocks × block_bytes` so every concurrent
/// sequence gets a fixed, budgeted sub-range — replacing the prior per-state
/// lazy allocation that OOMed at c≥8 (each of `num_slots × layers` pools was a
...
pub struct DeepseekBatchDecodeBuffers {
    ...
    /// Shared owning FP8 KV pool, or `None` when the FlashMLA decode env knob is
    /// off (legacy `dsv4_hybrid_attention_cuda` decode does not use it).
    fp8_kv_pool: Option<CudaSlice<u8>>,
    /// Number of `num_slots × layers` sub-ranges the pool was sized for.
    fp8_kv_slots: usize,
    fp8_kv_layers: usize,
...
impl DeepseekBatchDecodeBuffers {
    ...
    /// pool is not allocated (FlashMLA decode env knob off).
    ...
    /// is monotonic, mirroring the per-state `ensure_*` it replaces).
