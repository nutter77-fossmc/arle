...
def main() -> None:
    ...
    summary = {
        ...
        "fwht_bug_control": {
            ...
            "interpretation": (
                "ARLE CUDA output matches the current kernel FWHT sign convention after BF16 "
                "rounding; it does not match scripts/turboquant_weights.py faithful FWHT."
            ),
        },
        "decision": (
            "cuda_dequant_differs_from_python"
            if reports[2]["max_abs"] > 0.0
            else "cuda_dequant_matches_python"
        ),
    }
