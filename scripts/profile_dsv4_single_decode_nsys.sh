...
Environment:
  ...
  ARLE_DSV4_EXPERT_BACKEND, ARLE_DSV4_INCREMENTAL_KV,
  ARLE_DSV4_FUSED_DISPATCH_PAYLOAD, and ARLE_CUDA_ALLOC_TRACE are read from
  the environment. The DSv4 runtime knobs are defaulted for 8xH20
  DeepEP/DeepGEMM runs; allocation tracing stays default-off unless explicitly
  set.
EOF
}
