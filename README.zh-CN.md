...
- **2026-05-10** — W4-hybrid prefill graph capture 用 Path B.2 bucketed
  allocation key 关掉 4k/c=4 SGLang +76.6% gap。Engine TTFT p50
  **2000 → 150 ms (-92.5%)**，吞吐 **+632%** 在 60s（RTX 4070 Ti SUPER 16GB）；
  capture-key churn **388 → 7 unique** (98.5% LRU 复用)。开关：
  `INFER_PREFILL_GRAPH=1` + `INFER_HYBRID_W4A8_PREFILL=1`。同期 RoPE
  YARN/Linear/NtkAware scaling 落地。
  证据：[`docs/experience/wins/2026-05-10-bench-40-pathB2-tier1-strong-proceed.md`](docs/experience/wins/2026-05-10-bench-40-pathB2-tier1-strong-proceed.md)。
