...
| **OPD 训练（CUDA）** | Linux + NVIDIA | **Beta** | 真实 Qwen3-0.6B checkpoint + RTX 4070 Ti SUPER 上 OPD step 端到端 **0.164 s/step**（~170× 起始 naive 基线），同形态下 PyTorch CUDA 参考 83 ms → ARLE moderate **48.5 ms / 1.71× 领先**。CPU/CUDA loss bit-equivalent（relerr ~1.3e-6）；lr=1e-7 5k 步训练上 held-out exact-overlap 由 **50 → 82.8 %**，KL/NLL 单调下降未到 plateau。 |
