...
| `arle train opd` | **On-Policy Distillation** —— 仓内唯一训练入口（teacher 跑在 `infer`，student 用 LoRA 或全量微调，共用同一运行时）。CUDA 路径 Qwen3-0.6B **0.164 s/step**（RTX 4070 Ti SUPER）；跨 runtime Qwen3.5-4B → Qwen3.5-0.8B LoRA 路径 **5.66 s/step**，200 步 KL 单调下降。使用手册：[`docs/projects/2026-05-21-arle-opd-cuda-usage-manual.md`](docs/projects/2026-05-21-arle-opd-cuda-usage-manual.md)。Pretrain / SFT / GRPO / multi-turn RL 于 2026-05-18 下线（[原因](docs/projects/2026-05-18-opd-only-pivot.md)）。 |
...
- **2026-05-21** — **ARLE OPD CUDA：更快 + 更省显存 + 同等质量,对比 HuggingFace TRL。**
  同 Qwen3-0.6B teacher/student、同 32 prompts、同 `rollout_len=8`、同 `lr=1e-7`、
  同 500 步、同 AdamW 配置、同 RTX 4070 Ti SUPER 卡。

  ![ARLE OPD CUDA vs HuggingFace TRL — 速度、显存、held-out KL 三项 head-to-head](docs/projects/img/2026-05-21-arle-vs-pytorch-opd-comparison.png)

  | | PyTorch TRL `GKDTrainer` | **ARLE 全量微调** | **ARLE LoRA r=16** |
  |---|---:|---:|---:|
  | step 时间 (s) | 0.408 | **0.164** (快 2.49×) | **0.140** (快 2.91×) |
  | 显存峰值 (GB) | 12.6 | 15.4 | **3.93** (4 GB 显卡可跑) |
  | held-out KL 改善 (500 步) | -5.5 % | **-18.5 %** | **-36.4 %** |

  **大 teacher 路径已经穿过生产 runtime 边界实测。** Qwen3.5-4B BF16 teacher
  跑在 `infer`，Qwen3.5-0.8B-Base LoRA r=16 student 跑在 `train`，通过
  `InferTeacher` device-logits bridge 对接。同一张 RTX 4070 Ti SUPER 上，
  200 步真实文本 run 的 mean step 为 **5.66 s**，显存峰值 **14.8 GiB**，
  KL 单调下降：

  | 配置 | Step 时间 | 显存峰值 | 200 步 KL |
  |---|---:|---:|---:|
  | Qwen3.5-4B teacher → Qwen3.5-0.8B LoRA student | **5.66 s** | **14.8 GiB** | train **-1.66 %**，held-out **-2.05 %** |

  这是方向性与系统路径验证，不是最终质量 headline：9B 级 teacher 仍依赖量化
  infer-teacher 路径，真正 benchmark 质量结论还需要更长训练与更完整 eval。

  **用户支付什么、获得什么。** 迁移成本可控 —— ARLE 复用了 HuggingFace
  用户已有的所有产物,切换面只是把 `accelerate launch` 换成 `arle train opd`。

  | 不变的 | 变的 |
  |---|---|
  | HuggingFace `safetensors` 模型权重 | `arle train opd` CLI 替代 `accelerate launch trl_train.py` |
  | checkpoint 自带的 `tokenizer.json` | 一次构建:`cargo build --release --features cuda` |
  | 标准 OPD 超参数 (lr、betas、ε、grad clip) | 热路径不需要 `transformers` / `torch` Python 依赖 |
  | JSONL prompt 文件 (`--prompts-file`) | LoRA 模式不用调显存 (3.9 GB 峰值,4 GB 卡够) |
  | 业界 benchmark (MMLU、IFEval、lm-eval-harness) | 一个 binary、一行 shell、cargo 构建之外不用配环境 |

  **端到端收敛已验证。** held-out exact-overlap **50 % → 82.8 %**(5000 步、
  lr=1e-7);KL 在 10000 步内持续单调下降。

  ![ARLE OPD CUDA — Qwen3-0.6B 收敛曲线 (lr=1e-7 下 5000 步 held-out 由 50 升至 82.8%)](docs/projects/img/2026-05-21-opd-cuda-convergence.png)

  工程证据(32-commit kill-or-license 循环 + 对照 TRL 测试 + LoRA bench + 收敛实验):
